...
import { useGLTF, Detailed, Environment, Loader } from "@react-three/drei";
// https://github.com/pmndrs/react-postprocessing
// https://github.com/vanruesc/postprocessing
import { EffectComposer, DepthOfField } from "@react-three/postprocessing";

function Guitar({ index, z, speed }) {
  const ref = useRef();
  ...
  const { nodes, materials } = useGLTF("./guitColor.glb");
  ...
  return (
    <Detailed ref={ref} distances={[0, 65, 50]}>
      <group dispose={null}>
        <mesh
          castShadow
          receiveShadow
        ...
        <mesh
          castShadow
          receiveShadow
          geometry={nodes.Plane010_10.geometry}
...
export default function Guitars({
  ...
  return (
    ...
    <Canvas
      gl={{ antialias: false }}
      dpr={[1, 1.5]}
      camera={{ position: [0, 0, 10], fov: 130, near: 0.01, far: depth + 200 }}
    >
      <spotLight
        position={[10, 20, 10]}
        penumbra={1}
      ...
      {Array.from(
        { length: count },
        (_, i) => <Guitar key={i} index={i} z={Math.round(easing(i / count) * depth)} speed={speed} /> /* prettier-ignore */
      )}
